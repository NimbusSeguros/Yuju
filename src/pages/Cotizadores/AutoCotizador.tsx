...
const GroupedQuoteCard = ({ group, isCreditCard, getInsurerLogo, onInfo, onContract }: any) => {
  const [selectedIdx, setSelectedIdx] = useState(() => {
    const fivePercent = group.findIndex((q: any) => q.cobertura.includes('5%'));
    return fivePercent !== -1 ? fivePercent : 0;
  });

  const cot = group[selectedIdx];
  const logoUrl = getInsurerLogo(cot.aseguradora);

  return (
    <div className="bg-bg-primary border-2 border-border-primary/50 rounded-3xl p-5 hover:border-yuju-blue hover:shadow-[0_0_25px_rgba(51,105,255,0.2)] transition-all duration-500 relative flex flex-col min-h-[240px] group/card">
      <div className="flex justify-between items-start mb-4">
        <div className="flex-1 pr-6 min-h-[40px]">
          {group.length > 1 ? (
            <div className="relative group/select">
              <select 
                value={selectedIdx}
                onChange={(e) => setSelectedIdx(parseInt(e.target.value))}
                className="w-full bg-bg-secondary/80 border border-yuju-blue/20 rounded-xl px-3 py-2 text-[10px] font-black text-text-primary leading-tight cursor-pointer focus:outline-none appearance-none pr-8 hover:border-yuju-blue/50 hover:bg-bg-secondary transition-all shadow-sm"
              >
                {group.map((q: any, i: number) => (
                  <option key={i} value={i} className="bg-bg-primary text-text-primary py-2 italic font-bold">{q.cobertura}</option>
                ))}
              </select>
              <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-yuju-blue opacity-60 group-hover/select:opacity-100 transition-all scale-110">
            ...
            <h4 className="text-[11px] font-black text-text-primary leading-tight">{cot.cobertura}</h4>
          )}
        </div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="relative group/tooltip">
          <button onClick={() => onInfo(cot)} className="p-1.5 rounded-full bg-bg-secondary text-text-secondary group-hover/card:text-yuju-blue group-hover/card:bg-yuju-blue/5 border border-border-primary transition-all shadow-sm shrink-0">
            <Info size={16} />
          </button>
        </div>
      </div>
      ...
      <div className="mt-auto">
        <Button className="w-full h-11 rounded-xl font-black text-[10px] tracking-widest bg-yuju-blue hover:bg-blue-700 border-none flex items-center justify-center gap-2 shadow-lg shadow-yuju-blue/10" onClick={() => onContract(cot)}>
...
export const AutoCotizador = () => {
  ...
  const groupQuotes = (quotes: any[]) => {
    const groups: { [key: string]: any[] } = {};
    quotes.forEach(q => {
      // Limpiamos profundamente el nombre para agrupar planes de RUS (T31/T34), ATM, etc.
      const baseName = q.cobertura
        .replace(/\d+%\s*(de la SA|SUMA ASEGURADA|VARIABLE|C\/FCIA\.?|FRANQUICIA)?/gi, '') // Quita porcentajes y palabras asociadas
        .replace(/MINIMO\s*\$\s*[\d.]+/gi, '') // Quita el texto de MINIMO $... de Meridional
        .replace(/\(T\d+\)|T\d+/gi, '') // Quita códigos como (T31), T34, etc.
        .replace(/\s+/g, ' ')           // Normaliza espacios
        .replace(/-\s*$/i, '')          // Quita guiones al final
        .trim();
      
      const key = `${q.aseguradora}_${baseName}`;
  ...
  const handleSubmitLead = async () => {
    ...
    try {
      const payload = {
        ...
        pago_con_tarjeta: isCreditCard,
      };

      // Guardamos en Supabase a través de nuestro backend
      await apiClient.post('/cotizaciones', payload);

      // Cerramos modal y pasamos al paso de éxito (SuccessStep)
